...
class CrossRefExportPlugin extends ImportExportPlugin {
	...
	function display(&$args) {
		...
		//var_dump(config::getVar('general','base_url['.$journal->getPath().']'));exit;

		switch (array_shift($args)) {							
			case 'exportIssues':
				$issueIds = Request::getUserVar('issueId');
				$outputFormat = Request::getUserVar('type_articles');
				...
				$this->exportIssues($journal, $issues,null,$outputFormat);
				break;
			case 'exportIssue':
			...
			case 'exportArticles':
				...
				$outputFormat = Request::getUserVar('type_articles');
				$domainURL = Request::getUserVar('domain_article');
				if(strlen(trim($domainURL))==0)
				{
					$domainURL=null;
				}
				if (!isset($articleIds)) $articleIds = array();
				$results =& ArticleSearch::formatResults($articleIds);
				$this->exportArticles($journal, $results,null,$outputFormat,$domainURL);
				break;
			case 'issues':
	...
	function exportArticles(&$journal, &$results, $outputFile = null,$outputFormat = 'article') {
		...
		foreach ($results as $result) {
			...
			/*
			 * If output format is article, add journal header and issue
			 */
			switch ($outputFormat) {
				case 'article':
				...
				case 'report':
					$journalNode =& XMLCustomWriter::createElement($doc, 'report-paper');
					
					// Create the article
					$journalArticleNode =& CrossRefExportDom::generateJournalReportDom($doc, $journal, $issue, $section, $article);
					XMLCustomWriter::appendChild($journalNode, $journalArticleNode);
	...
	function exportIssues(&$journal, &$issues, $outputFile = null,$outputFormat = 'article') {
		$this->import('CrossRefExportDom');
